...
import requests, re, datetime, logging, time
...
def format_custom_time(dt_str):
    ...
    formatted_time = formatted_time[:-2] + formatted_time[-2:].lower()
    return formatted_time

def format_time(ts):
    """ Converts an epoch timestamp (or numeric string) into a formatted time string. """
    try:
        if ts == "Not Found":
            return ts
        ts_int = int(ts)
        ...
        return format_custom_time(dt_str)
...
def get_skill_info(skill_id):
    """
    Returns the mapping information for a given skill ID.
    If not found, returns a default dict.
    """
...
def format_equipped_skills(equipped_skills):
    """
    Converts each equipped skill ID into its corresponding character name.
    If the skill ID is 8, 16, 3, 2, or 1, it is skipped.
    The first valid skill is marked with (P) and the rest with (A).
    Returns a string that lists the characters with their respective markers.
    """
    if not equipped_skills:
        return 'Not Found'
    
    # Define a set of skill IDs (as strings) to skip.
    skip_ids = {"8", "16", "3", "2", "1"}
    filtered_skills = []
    ...
    for skill in equipped_skills:
        ...
        if skill_str in skip_ids:
            continue  # Skip these skill IDs.
        filtered_skills.append(skill_str)
    
    # Build the formatted list using (P) for the first item and (A) for the rest.
    formatted_list = []
...
def fetch_info():
    start_time = time.time()  # Start time for measuring response duration
    ...
    formatted_skills = format_equipped_skills(equipped_skills)    

    # Format the creation and last login times using our new format_time function.
...
"""
    ...
    if total_time < 0:    
        time.sleep(0 - total_time)    
        total_time = time.time() - start_time    

    final_response = f"""{text_response}

Total Time Taken: {total_time:.2f} seconds """
    return Response(final_response, mimetype='text/plain')
